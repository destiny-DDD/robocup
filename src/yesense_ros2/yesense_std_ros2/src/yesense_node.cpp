...
public:
  YESENSE_Publisher() : Node("yesense_publisher") {
    ...
    pub_imu_ros = this->create_publisher<sensor_msgs::msg::Imu>(
        imu_topic_ros.c_str(), 10);
    pub_pose_geometry =
    ...
    // ==========================================
    pub_imu = this->create_publisher<yesense_interface::msg::ImuData>(
        imu_topic.c_str(), 10);
    pub_sensor_10axis =
    ...
    *((unsigned int *)&yis_out) = 0u;
    ...
    timer_ = this->create_wall_timer(
        1ms, std::bind(&YESENSE_Publisher::timer_callback, this));
    timer_msg_rate_ = this->create_wall_timer(
        1ms, std::bind(&YESENSE_Publisher::callback_msg_rate_calc, this));
  }
    ...
    if (serial_drv_ros == driver_type) {
      if (ser.isOpen() && ser.available()) {
        size_t bytes_to_read =
            std::min(static_cast<size_t>(ser.available()), sizeof(r_buffer));
        size_t available_bytes = ser.available();
        // std::cout << "\033[1m\033[34m" << "wheeltec_tues debug_aaaaaaaa" <<
        // "\033[0m"<< std::endl;  // wheeltec_tues debug
